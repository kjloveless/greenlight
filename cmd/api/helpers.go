...
func (app *application) readJSON(
	...
	// Initialize the json.Decoder, and call the DisallowUnknownFields() method
	// on it before decoding. This means that if the JSON from the client now
	// includes any field which cannot be mapped to the target destination, the
	// decoder will return an error instead of just ignoring the field.
	...
	if err != nil {
		...
		// errors.Is() and return a generic error message. There is an open issue
		// regarding this at https://github.com/golang/go/issues/25956.
		...
		case errors.As(err, &unmarshalTypeError):
			...
			return fmt.Errorf(
				"body contains incorrect JSON type (at character %s)",
				unmarshalTypeError.Field)
		...
		// "json: unknown field "<name>"". We check for this, extract the field
		// name from the error, and interpolate it into our custom error message.
		// Note that there's an open issue at
		// https://github.com/golang/go/issues/29035 regarding turning this into a
		// distinct error type in the future.
		...
		// rather than returning an error to our handler. At the end of this
		// chapter we'll talk about panicking versus returning errors, and discuss
		// why it's an appropriate thing to do in this specific situation.
	...
	// Call Decoder() again, using a pointer to an empty anonymous struct as the
...
// destination http.ResponseWriter, the HTTP status code to send, the data to
// encode to JSON, and a header map containing any additional HTTP headers we
...
func (app *application) writeJSON(
	...
	// writing the response, so its's safe to add any headers that we want to
