...
func (app *application) showMovieHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // from the slice. In our project all movies will have a unique positive
  // integer ID, but the value returned by ByName() is always a string. So we
  // try to convert it to a base 10 integer (with a bit size of 64). If the
  // parameter couldn't be converted, or is less than 1, we know the ID is
  // invalid so we use the http.NotFound() function to return a 404 Not Found
  // response.
  ...
  // Call the Get() method to fetch the data for a specific movie. We also need
  // to use the errors.Is() function to check if it returns a
...
func (app *application) updateMovieHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // If the input.Title is nil then we know that no corresponding "title"
  ...
  // with the new title value. Importantly, because input.Title is now a
  // pointer to a string, we need to dereference the pointer using the *
  ...
  // Intercept any ErrEditConflict error and call the new
  // editConflictResponse() helper.
...
func (app *application) listMoviesHandler(w http.ResponseWriter, r *http.Request) {
  // To keep things consistent with out other handlers, we'll define an input
  ...
  // falling back to defaults of an empty string and empty slice respectively
