...
// The errorRequest() method is a generic helper for sending JSON-formatted
// error messages to the client with a given status code. Note that we're using
// the any type for the message parameter, rather than just a string type, as
// this gives us more flexibility over the values that we can include in the
// response.
